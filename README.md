...
| パッケージ | 役割 | 依存 |
|---|---|---|
| `@poc/core` | Result 型・基本エンティティ | なし |
| `@poc/utils` | JSON/ID などの共通ヘルパー | core |
| `@poc/client` | 上2つを合成した高レベル API | core, utils |
...
- `.github/workflows/release.yml` … main への push で Changesets が
  「Version Packages」PR を自動生成 / 未消化 changeset が無ければ npm publish
  - 公開には repo secret **`NPM_TOKEN`** の設定が必要

## リリースフロー

1. PR で `pnpm changeset` を実行し semver レベルを記録
2. main マージで Changesets が "Version Packages" PR を自動生成
3. その PR をマージ → 変更のあったパッケージのみ npm publish
